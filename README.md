...
<br>
To get the Twitch Token, do the following steps:
...
<br><br>
Type the twitch channel's name the bot is supposed to be used on into the field "Twitch Channel". Note that the bot should moderator on that channel.
<br><br>
Both the Spotify ID and Spotify the Secret need to get copied from the [Spotify Developer Dashboard](https://developer.spotify.com/dashboard).
Login with your spotify credentials and do the following steps to get them:<br>
...
Now the data-folder should exist with the following structure:
````
-> data
    -> properties
...
<br><br>
#### Update Properties
Now we need to fill the files with the necessary properties. For that, execute the UpdateProperties.jar of the newest project version.
After doing that, a cmd will open and log the added properties:<br>
![updateProperties.png](images/updateProperties.png)
<br>
The properties-files in folder data\properties now have the necessary entries. They only have default values at the moment.
<br>
...
Now we need to set up the Spotify token. Execute the latest version of SetupToken.jar. Two things will happen: A Browser Tab opens, and the following window will be visible:<br>
![setupToken.png](images/setupToken.png)
<br>
Go into the browser tab. It has the website of the redirect URI open, example.com. In the URL, you will see the following text:<br>
...
On startup, the bot will display a message in chat to confirm start up. <br>

## How to Update
If you have the update-checker enabled and a new version is available, a window is displayed on startup of the Spotify-Bot. There you can either:
* Click on "Update" and the bot automatically updates itself or
* Click on "Go to GitHub" and update it manually by downloading all the necessary assets and the new version.

The automatic update will download the latest version of the update-script if it is not already present in a "bin"-folder and then execute it.
<br><br>
If you still want to manually update the Spotify-Bot, you can click on "Go to GitHub" or find it under [Releases](https://github.com/alexshadowolex/spotify-bot/releases). In every release, you can find a changelog for big changes and some attached files.
<br>
If the release includes a file "UpdateProperties_<version>.jar" it means that new properties have been added during the release. Download this file and execute it first, so all necessary properties are existing. The cmd will inform you, which properties in which files are new.
<br>
...
* Song Name Getter -> Can be toggled off (switch in UI under "General Settings"). Gets the current song name, artists and album image and writes it into different files:
  ...
  * data\displayFiles\currentRequestedByText.txt -> Current requested by text, if the song was requested by a user
  <br><br>
...
* Update Checker -> Can be toggled off (switch in UI under "General Settings").<br>
  Checks for new versions on GitHub, and if there is one, a window will appear and show the new changes.<br>
  In the window, there is a button to auto-update the bot.
...
* Remove Song From Queue Command -> Can be toggled off (switch in UI under "General Settings").<br>
  ...
  To use this command, type the song name and artists and the bot searches for the most similar song in the queue. To achieve the best results, use the song name string that the bot responds with after a song request (or when you use the song info command), e.g. ""Ghost" by Confetti"<br>
  This functionality can be restricted to be used only by certain user groups:
  * BROADCASTER: Only the broadcaster
  * MODERATOR: Owner and Moderators
...
* If any issues occurred, you can do the following to let me know (please also provide the log files and tell me what went wrong):
  * Create an issue on [GitHub](https://github.com/alexshadowolex/Spotify-Bot/issues)
  * Message me on Discord (alexshadowolex)
  * Message me on [Twitch](https://twitch.tv/alexshadowolex)
